...
import { mainMenuList, productsList, productsActs, cartActs, cardNumberActs, cardOwnerActs, cardCVVActs, productsListInstance } from './constants';
...
let amountItemsInBasket: number = 0;
let basketOfGoods: productsListInstance[] = [];
...
function productPage() {
    const allPossibleProductPageActions: productsListInstance[] = productsList.concat(productsActs);
    const selectedAction: number = parseInt(products.action());
...
function cartPage(): void {
    const allPossibleProductPageActions: productsListInstance[] = basketOfGoods.concat(cartActs);
...
function cartOwnerPage(): void {
    const cartOwner: string = menuOwnerName.action();
...
function cartNumberPage(): void {
    const cartNumber: string = menuCardNumber.action();
...
function mainPage() {
    const firstCall: string = mainMenu.action();
    ...
    let returnedFunction: void = (firstCall === '1' || firstCall === '2') ? chooseFunc(firstCall) : wrongAnswer();
...
function passwordPage() {
    const userPassword: string = password.action();
...
function loginPage() {
    const userName: string = login.action();
